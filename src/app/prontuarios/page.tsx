...
export default function ProntuariosPage() {
  ...
  const fetchMedicalRecords = async () => {
    try {
      ...
      if (result.success) {
        ...
        console.log(`✅ Frontend - Carregados ${result.data.length} prontuários da clínica "${user.tenant?.name}"`)
      } else {
  ...
  const filteredRecords = medicalRecords.filter(record => {
    ...
    const matchesProfessional = !filterProfessional || filterProfessional === 'all' || record.professional.id === filterProfessional
    const matchesServiceType = !filterServiceType || filterServiceType === 'all' || record.serviceType === filterServiceType
  ...
  const profissionaisUnicos = [...new Set(medicalRecords.map(r => r.professional.id))].length
  const pacientesAtendidos = [...new Set(medicalRecords.map(r => r.patient.id))].length
    ...
    <MainLayout breadcrumbs={breadcrumbs}>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Prontuários e Evolução</h1>
        ...
        <Card>
          ...
          <CardContent>
                    ...
                    <TableRow key={record.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end space-x-2">
                          ...
                          <EditarProntuarioForm record={record} onSuccess={fetchMedicalRecords} />
                          <DeleteProntuarioDialog record={record} onSuccess={fetchMedicalRecords} />
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
            ...
            {!loading && !error && medicalRecords.length > 0 && filteredRecords.length === 0 && (
              <div className="text-center py-8">
