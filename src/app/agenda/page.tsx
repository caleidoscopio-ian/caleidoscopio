...
import { Calendar, Clock, Plus, Filter, ChevronLeft, ChevronRight, User, Users } from 'lucide-react'
...
import { format, addDays, subDays, startOfDay, endOfDay, startOfWeek, endOfWeek } from 'date-fns'
...
export default function AgendaPage() {
  ...
  const [profissionais, setProfissionais] = useState<Array<{ id: string; nome: string; especialidade: string }>>([])
  ...
  const loadPacientes = async () => {
    try {
      ...
      setPacientes(data.map((p: any) => ({ id: p.id, nome: p.name || p.nome })))
    } catch (error) {
  ...
  const handleIniciarAtendimento = (id: string) => {
    ...
    toast({
      ...
      description: 'Funcionalidade de iniciar atendimento será implementada em breve'
    })
  }

  const handleNovoAgendamentoClick = (profissionalId: string, horario: string) => {
  ...
  const handleNovoAgendamentoSemanalClick = (data: Date, horario: string) => {
    setNovoAgendamentoDefaults({
      profissionalId: selectedProfissional !== 'all' ? selectedProfissional : undefined,
      data,
      horario
    })
    setShowNovoAgendamento(true)
  ...
  const agendamentosHoje = agendamentos.filter(a =>
    format(new Date(a.data_hora), 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd')
  )

  const profissionaisFiltrados = selectedProfissional === 'all'
    ? profissionais
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Button variant="outline" onClick={() => setShowNovoAgendamento(true)}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-purple-600">
                {agendamentos.filter(a => a.status === StatusAgendamento.ATENDIDO).length}
              </div>
            </CardContent>
          </Card>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div className="text-lg font-semibold capitalize">
                  {format(selectedDate, viewMode === 'day' ? "EEEE, d 'de' MMMM 'de' yyyy" : "'Semana de' d 'de' MMMM", { locale: ptBR })}
                </div>
          ...
          <CardContent className="p-0">
            {viewMode === 'day' ? (
              <AgendaDiaria
                ...
                showMultipleProfessionals={isAdmin && selectedProfissional === 'all'}
              />
            ) : (
              <AgendaSemanal
                agendamentos={agendamentos}
