...
export function AgendaDiaria({
  ...
  const isSlotOccupied = (profissionalId: string, slotTime: string): Agendamento | null => {
    const agendamentosDoProfissional = agendamentosPorProfissional.get(profissionalId) || []
  ...
  const isStartSlot = (agendamento: Agendamento, slotTime: string): boolean => {
    ...
    return agendamentoDate.getHours() === slotHour && agendamentoDate.getMinutes() === slotMinute
  }
  ...
  const profissionaisExibir = showMultipleProfessionals ? profissionais : profissionais.slice(0, 1)

  return (
    <div className="border rounded-lg overflow-hidden">
      {/* Header com profissionais */}
      <div className="grid" style={{ gridTemplateColumns: `80px repeat(${profissionaisExibir.length}, 1fr)` }}>
        {/* Coluna de horários */}
        ...
        {profissionaisExibir.map(prof => (
          <div key={prof.id} className="bg-muted p-4 border-r last:border-r-0 border-b">
      ...
      <div className="overflow-y-auto max-h-[600px]">
        {TIME_SLOTS.map((slot, slotIndex) => (
          <div
            ...
            style={{ gridTemplateColumns: `80px repeat(${profissionaisExibir.length}, 1fr)` }}
          >
            {/* Coluna de horário */}
            ...
            {profissionaisExibir.map(prof => {
              ...
              const isStart = agendamento ? isStartSlot(agendamento, slot) : false

              // Se há agendamento mas não é o slot inicial, não renderizar (já foi renderizado no slot inicial)
              if (agendamento && !isStart) {
                return <div key={`${prof.id}-${slot}`} className="border-r last:border-r-0" />
              }
                    ...
                    >
                      <div className="space-y-1 h-full overflow-hidden">
                        ...
                        {slotsOcupados > 1 && (
                          <Badge
                            variant="outline"
                            className={cn('text-xs inline-block', getStatusBadgeColor(agendamento.status as StatusAgendamento))}
                          >
