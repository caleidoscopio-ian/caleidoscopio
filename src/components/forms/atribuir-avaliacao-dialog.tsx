...
interface AtribuirAvaliacaoDialogProps {
  avaliacao: Avaliacao;
  onSuccess: () => void;
}

export function AtribuirAvaliacaoDialog({
  avaliacao,
  onSuccess,
}: AtribuirAvaliacaoDialogProps) {
  const { user } = useAuth();
  ...
  // Atribuir avaliação aos pacientes selecionados
  const handleAtribuir = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Erro ao atribuir avaliação"
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh]">
        <DialogHeader>
          <DialogTitle>Atribuir Avaliação a Pacientes</DialogTitle>
          <DialogDescription>
            Selecione os pacientes que receberão esta avaliação
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 border rounded-lg bg-muted/30">
            ...
            <div className="text-sm text-muted-foreground mt-1">
              <Badge variant="outline">{traduzirTipo(avaliacao.tipo)}</Badge>
            </div>
          </div>
