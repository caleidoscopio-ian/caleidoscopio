...
import { CalendarIcon, Loader2, Edit, Plus, Trash2 } from "lucide-react";
...
export function EditarPacienteForm({
  ...
  // Fallback: paciente antigo sem PacienteResponsavel, mas com guardianName legado
  const responsaveisIniciais =
    patient.responsaveis && patient.responsaveis.length > 0
      ? patient.responsaveis.map((r) => ({
          nome: r.nome,
          telefone: r.telefone || "",
          parentesco: r.parentesco as "PAI" | "MAE" | "TUTOR_LEGAL" | "AVO" | "IRMAO" | "OUTRO",
          cpf: r.cpf || "",
  ...
  // Fallback: paciente antigo sem PacienteConvenio, mas com convenioId legado
  const conveniosIniciais =
    patient.convenios && patient.convenios.length > 0
      ? patient.convenios.map((c) => ({
        ...
        ? [
            {
              ...
              numero_carteirinha: patient.healthInsuranceNumber || "",
              principal: true,
            },
          ]
        : [];
  ...
  const form = useForm<PacienteFormData>({
    ...
    defaultValues: {
      ...
      sexo: (patient.sexo as PacienteFormData["sexo"]) || undefined,
      cpf: patient.cpf,
      email: patient.email || "",
