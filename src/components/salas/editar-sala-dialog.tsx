  ...
  const onSubmit = async (data: SalaFormData) => {
    try {
      ...
      const response = await fetch("/api/salas", {
        ...
        body: JSON.stringify({ id: sala.id, ...data, recursos, filialId: data.filialId || null }),
      });

      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Erro ao atualizar sala");
      ...
      toast({
        title: "Erro",
        description: error instanceof Error ? error.message : "Erro ao atualizar sala",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Atualize os dados da sala. Campos com * são obrigatórios.</DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField control={form.control} name="ativo" render={({ field }) => (
              <FormItem className="flex items-center justify-between rounded-lg border p-4">
                <div className="space-y-0.5">
                  ...
                  <FormDescription>Salas inativas não aparecem no agendamento</FormDescription>
                </div>
                <FormControl>
            ...
            <div className="flex justify-end gap-2 pt-4 border-t">
              ...
              <Button type="submit" disabled={loading}>
                {loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Salvando...</> : <><Edit className="mr-2 h-4 w-4" />Salvar Alterações</>}
              </Button>
            </div>
          </form>
        </Form>
